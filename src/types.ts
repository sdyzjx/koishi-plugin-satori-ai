...
    }).description('基础设置'),
    ...
    // 新增视觉模型配置 Schema
    Schema.object({
      vision_LLM_URL: Schema.string().description('视觉模型请求地址 (例如: https://api.openai.com/v1)'),
      vision_LLM_key: Schema.union([
        Schema.array(String).role('secret'),
        Schema.transform(String, value => [value]),
      ]).role('secret').description('视觉模型 API Key'),
      vision_LLM: Schema.string().description('视觉模型名称 (例如: gpt-4-vision-preview)'),
    }).description('视觉模型设置'),
